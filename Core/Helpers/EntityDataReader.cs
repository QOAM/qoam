  ...
  <xs:element name=""NewDataSet"" msdata:IsDataSet=""true"" msdata:MainDataTable=""SchemaTable"" msdata:Locale="""">
    <xs:complexType>
      <xs:choice minOccurs=""0"" maxOccurs=""unbounded"">
        ...
        private static List<Attribute> scalarAttributesPlusRelatedObjectKeyAttributes;
        private static Type nullable_T = typeof(int?).GetGenericTypeDefinition();
        private readonly IEnumerator<T> enumerator;
        private readonly EntityDataReaderOptions options;
        ...
        {
            var s = new DataSet();
            s.Locale = CultureInfo.CurrentCulture;
        ...
        private static List<Attribute> DiscoverRelatedObjectKeyAttributes(Type thisType, ObjectContext objectContext)
        {
            ...
            var thisEntity = entityTypesByName[thisType.FullName].First();
            ...
            foreach (var pi in erProps)
            {
                ...
                var relatedEntityCLRTypeName = pi.PropertyType.GetGenericArguments().First().FullName;

                //Find the EntityType at the other end of the relationship because we need to get its key attributes.
                var relatedEntityEFType = entityTypesByName[relatedEntityCLRTypeName].FirstOrDefault();
                ...
                {
                    var targetKeyAttributeName = key.Name;
                    ...
                    else //there's no rule that the EntityReference named like this so, if not just use the target type
                    {
                        ...
                        var ix = erProps.Where(p => p.PropertyType == pi.PropertyType).ToList().IndexOf(pi);
                        if (ix > 0)
                        {
                            referenceName = referenceName + ix.ToString(CultureInfo.InvariantCulture);
                        }
                    }
                    var fullName = referenceName + "_" + key.Name;

                    //bind out local variables for the valueAccessor closure.
                    var kType = Type.GetType(key.TypeUsage.EdmType.FullName);
                    var entityReferenceProperty = pi;
                    ...
                    Func<T, object> valueAccessor = o =>
                        {
                            ...
                            var val = er.EntityKey.EntityKeyValues.First(k => k.Key == targetKeyAttributeName).Value;
                            return val;
                        };
                    var name = key.Name;
        ...
        private static List<Attribute> DiscoverRelatedObjectScalarAttributes(Type thisType)
        {
            ...
            foreach (var rop in relatedObjectProperties)
            {
                ...
                {
                    var attName = rop.Name + "_" + sp.Name;
                    ...
                    var name = attName;
                    var att = new Attribute(rop.Name, attName, sp.PropertyType, valueAccessor, true);
                    atts.Add(att);
                }
            }
            return atts;
        ...
        {
            var val = (TField)this.attributes[i].GetValue(this.current);
            return val;
        }
        ...
        private class Attribute
        {
            ...
            public static Func<TObject, TProperty> MakeRelatedPropertyAccessor<TObject, TProperty>(PropertyInfo pi, PropertyInfo pi2)
            {
                Func<TObject, object> getRelatedObject;
                {
                    ...
                    var typedParam = Expression.Parameter(typeof(T), "t");
                    var typedAccessor = Expression.PropertyOrField(typedParam, pi.Name);
                    var castToObject = Expression.Convert(typedAccessor, typeof(object));
                ...
                Func<object, TProperty> getRelatedObjectProperty;
                {
                    ...
                    var typedParam = Expression.Convert(objParam, pi.PropertyType);
                    var typedAccessor = Expression.PropertyOrField(typedParam, pi2.Name);
                    var castToObject = Expression.Convert(typedAccessor, typeof(TProperty));
