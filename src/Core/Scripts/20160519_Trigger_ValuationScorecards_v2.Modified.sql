...
AS BEGIN
    ...
    USING (
	    SELECT  
            ...
            ,CASE WHEN v.Number = 1 THEN CAST(COALESCE(SUM([Score_ValuationScore_TotalScore]), 0) AS FLOAT) / COALESCE(SUM(v.ValuationNumberOfQuestions), 1) ELSE 0 END AS [ValuationScore_AverageScore_v1]
			,CASE WHEN v.Number = 2 THEN CAST(COALESCE(SUM([Score_ValuationScore_TotalScore]), 0) AS FLOAT) / COALESCE(SUM(v.ValuationNumberOfQuestions), 1) ELSE 0 END AS [ValuationScore_AverageScore_v2]
        FROM [Journals] j
